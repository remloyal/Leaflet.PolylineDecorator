function init() {
      ...
      patterns: [
        {
          ...
          symbol: L.Symbol.dash({
            pixelSize: 5,
            pathOptions: { color: "#000", weight: 1, opacity: 0.2 },
          }),
        },
        {
          offset: "16%",
  ...
  for (var i = 0; i < 500; i++) {
    ...
    for (var j = 0; j < 2 + Math.random() * 10; j++) {
      points.push([-60 + Math.random() * 140, 10 + Math.random() * 160]);
    }
    bigDataCoords.push(points);
  }
    for (var i = 0; i < bigDataCoords.length; i++) {
      linesData.push(
        L.polyline(bigDataCoords[i], {
          lineJoin: "round",
          color: "#5b2ec4",
          fillOpacity: 0.5,
          opacity: 0.5,
  ...
  const gtStyleArrow = L.Symbol.arrowHead({
    ...
    pathOptions: {
      ...
      weight: 1.5,
      fill: false,
      renderer: polylineDecoratorRenderer, // 使用 canvas，减少 SVG path 节点数量
    },
  });
